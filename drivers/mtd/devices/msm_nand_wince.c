...
uint32_t flash_read_id(struct msm_nand_chip *chip)
{
	...
	msm_dmov_exec_cmd(chip->dma_channel, crci_mask, DMOV_CMD_PTR_LIST |
			  DMOV_CMD_ADDR(msm_virt_to_dma
					(chip, &dma_buffer->cmdptr)));
	dsb();
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	printk("msm_nand_read_oob [%08X %08X] %08X %08X %d %s\n",
	       ...
	       (uint32_t) ops->oobbuf, oob_count,
	       readoob ? oobType : "OOB_NONE");
#endif
	...
	if (readoob && !readdata) {
		...
		if (oob_count % page_count != 0) {
			pr_err
			    ("%s: OOB num pages != DATA num pages oob_count=%d\n",
			     __func__, oob_count);
			return -EINVAL;
		}
...
#if VERBOSE
	printk
	    ("msm_nand_read_oob ops->len=%d oob_count=%d page_count=%d page_oob_count=%d\n",
	...
	if (readdata) {
		...
		if (dma_mapping_error(chip->dev, data_dma_addr)) {
			pr_err
			    ("msm_nand_read_oob: failed to get dma addr for %p\n",
			     ops->datbuf);
			return -EIO;
		}
	}
	if (readoob) {
		...
		oob_dma_addr_curr = oob_dma_addr =
		    msm_nand_dma_map(chip->dev, ops->oobbuf, oob_count,
				     DMA_BIDIRECTIONAL);
	...
	while (page_count-- > 0) {
		...
		for (n = start_sector; n < cwperpage; n++) {
			...
			cmd->dst =
			    msm_virt_to_dma(chip, &dma_buffer->data.result[n]);
...
#if VERBOSE
				printk
				    ("msm_nand_read_oob oob_dma_addr_curr=%08x cwoobsize=%d page_oob_done=%d page_oob_count=%d\n",
				     (uint32_t) oob_dma_addr_curr, cwoobsize,
				     page_oob_done, page_oob_count);
#endif
			...
			if (readdata) {
...
#if VERBOSE
						printk
						    ("msm_nand_read_oob data pageerr=rawerr at n=%d\n",
						     n);
#endif
						break;
					}
				...
				if (data_count > 0) {
...
#if VERBOSE
					printk
					    ("msm_nand_read_oob dma_map_page offset=%d length=%d\n",
					     offset, data_count);
#endif
					data_dma_addr =
					    msm_nand_dma_map(chip->dev,
							     ops->datbuf +
							     offset, data_count,
							     DMA_FROM_DEVICE);
					data_dma_addr_curr = data_dma_addr;
					...
					if (dma_mapping_error
					    (chip->dev, data_dma_addr)) {
						pr_err
						    ("msm_nand_read_oob: failed to re-get dma addr "
...
#if VERBOSE
						printk
						    ("msm_nand_read_oob oob pageerr=rawerr at n=%d\n",
						     n);
#endif
						break;
					}
		...
		if (pageerr) {
			for (n = start_sector; n < cwperpage; n++) {
				if (dma_buffer->data.result[n].
				    buffer_status & 0x8) {
#if VERBOSE
					printk
					    ("msm_nand_read_oob oob pageerr ecc uncorrectable at n=%d\n",
					     n);
#endif
					/* not thread safe */
...
#if VERBOSE
					printk
					    ("msm_nand_read_oob oob pageerr ecc correctable n=%d ecc_errors=%d\n",
					     n, ecc_errors);
#endif
		...
		if (rawerr && !pageerr) {
			...
			pr_info
			    ("msm_nand_read_oob error status: %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x \n",
...
#if VERBOSE
			printk
			    ("msm_nand_read_oob: couldn't provide %d out of %d oob bytes, oob_done_size=%d\n",
			     page_oob_count - page_oob_done, page_oob_count,
			     oob_done_size);
#endif
	...
	if (readoob)
		dma_unmap_page(chip->dev, oob_dma_addr, oob_count,
			       DMA_BIDIRECTIONAL);
 err_dma_map_oobbuf_failed:
	...
	if (readoob && oob_done_size) {
		print_hex_dump_bytes("", DUMP_PREFIX_OFFSET, ops->oobbuf,
				     oob_done_size);
		printk("\n");
	}
	// dump data
	if (readdata && ops->len) {
		print_hex_dump_bytes("", DUMP_PREFIX_OFFSET, ops->datbuf,
				     ops->len);
	}
#endif
	if (readdata)
	...
	if (err)
		pr_err("msm_nand_read_oob %llx %x %x failed %d, corrected %d\n",
		       from, ops->datbuf ? ops->len : 0, oob_count, err,
		       total_ecc_errors);
#ifdef ENABLE_ENTRY_TRACE
	else
		printk("-&&&nand_readoob&&&\n");
...
msm_nand_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	...
	if (to < (NUM_PROTECTED_BLOCKS * mtd->erasesize)) {
		pr_err
		    ("%s: cannot write to page in protected area, to=0x%llx\n",
		     __func__, to);
		return -EINVAL;
	}
	if (ops->datbuf != NULL && (ops->len % mtd->writesize) != 0) {
	...
	printk("msm_nand_write_oob [%08X %08X] %08X %08X %d %s\n",
	       (uint32_t) (to), writedata ? (uint32_t) ops->len : 0,
	       (uint32_t) ops->datbuf, (uint32_t) ops->oobbuf, oob_count,
	       writeoob ? oobType : "OOB_NONE");
#endif
...
#if VERBOSE
	printk
	    ("msm_nand_write_oob ops->len=%d oob_count=%d page_count=%d page_oob_count=%d\n",
	     ops->len, oob_count, page_count, page_oob_count);
#endif
	...
	if (writedata) {
		...
		if (dma_mapping_error(chip->dev, data_dma_addr)) {
			pr_err
			    ("msm_nand_write_oob: failed to get dma addr for %p\n",
			     ops->datbuf);
			return -EIO;
		}
	}
	if (writeoob) {
		...
		if (!writedata) {
			data_dma_addr_curr = data_dma_addr =
			    msm_nand_dma_map(chip->dev, dummyud,
					     sizeof(dummyud), DMA_TO_DEVICE);
			if (dma_mapping_error(chip->dev, data_dma_addr)) {
				pr_err
				    ("msm_nand_write_oob: failed to get dma addr for %p\n",
				     dummyud);
			...
			dummyecc_dma_addr =
			    msm_nand_dma_map(chip->dev, dummyecc,
					     sizeof(dummyecc), DMA_TO_DEVICE);
		...
		for (n = 0; n < cwperpage; n++) {
			...
			if (writeoob && page_oob_done < page_oob_count) {
				...
				if (!writedata)	//if writing spare only, have to fill ecc with 0xFF
				{
					cmd->cmd = 0;
					cmd->dst =
					    MSM_NAND_FLASH_BUFFER + cwdatasize;
...
#if VERBOSE
				printk
				    ("msm_nand_write_oob oob_dma_addr_curr=%08x cwoobsize=%d page_oob_done=%d page_oob_count=%d\n",
				     (uint32_t) oob_dma_addr_curr, cwoobsize,
				     page_oob_done, page_oob_count);
#endif
				oob_dma_addr_curr += cmd->len;
			...
			cmd->cmd = 0;
			cmd->src =
			    msm_virt_to_dma(chip, &dma_buffer->data.zeroes);
		...
		for (n = 0; n < cwperpage; n++) {
			if (dma_buffer->data.flash_status[n] & 0x110) {
				if (dma_buffer->data.flash_status[n] & 0x10)
					pr_err
					    ("msm_nand: critical write error, 0x%x(%d)\n",
					     page, n);
...
#if VERBOSE
			printk
			    ("msm_nand_write_oob: no space to write %d out of %d oob bytes, oob_done_size=%d\n",
			     page_oob_count - page_oob_done, page_oob_count,
			     oob_done_size);
#endif
	...
	if (!writedata && writeoob)
		dma_unmap_page(chip->dev, dummyecc_dma_addr, sizeof(dummyecc),
			       DMA_TO_DEVICE);
 err_dummyecc_dma_failed:
	if (!writedata && writeoob)
		dma_unmap_page(chip->dev, data_dma_addr, sizeof(dummyud),
			       DMA_TO_DEVICE);
...
static int msm_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if (instr->addr < (NUM_PROTECTED_BLOCKS * mtd->erasesize)) {
		pr_err
		    ("%s: cannot erase block in protected area, addr=0x%llx\n",
		     __func__, instr->addr);
		return -EINVAL;
	}
...
static int msm_nand_block_isbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	dma_buffer->data.cfg1 =
	    MSM_NAND_CFG1_RAW | (chip->CFG1 & CFG1_WIDE_FLASH);
	...
	cmd->cmd = 0;
	cmd->src =
	    MSM_NAND_FLASH_BUFFER + (mtd->writesize - (528 * (cwperpage - 1)));
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	chip->CFG1 = chip->CFG1 & ~(1 << 0)	/* Enable ecc */
